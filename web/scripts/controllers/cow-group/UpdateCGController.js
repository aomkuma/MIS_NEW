angular.module('e-homework').controller('UpdateCGController', function($scope, $cookies, $filter, $state, $routeParams, $uibModal, HTTPService, IndexOverlayFactory) {
    ...
    $scope.loadData = function(action, id){
        ...
        $scope.Sperm.go_factory_weight = null;
        $scope.Sperm.go_factory_price = null;
        $scope.Sperm.go_factory_values = null;
        $scope.Sperm.cow_weight = null;
        $scope.Sperm.cow_price = null;
        $scope.Sperm.cow_values = null;
        $scope.Sperm.decline_weight = null;
        $scope.Sperm.decline_price = null;
        $scope.Sperm.decline_values = null;
        $scope.Sperm.yellow_milk_weight = null;
        $scope.Sperm.yellow_milk_price = null;
        $scope.Sperm.yellow_milk_values = null;
        $scope.Sperm.cow_group_avg = null;
    ...
    $scope.changeGroup = function(typeID, TypeList){   

        var typeName = '';

        for(var i = 0; i < TypeList.length; i++){
            // console.log(name , $scope.MasterGoalList[i].sub_goal_type);
        ...
        $scope.NewMasterGoalList = [];
        for(var i = 0; i < $scope.MasterGoalList.length; i++){
            console.log(typeName , $scope.MasterGoalList[i].sub_goal_type);
            if(typeName == $scope.MasterGoalList[i].sub_goal_type){
                $scope.NewMasterGoalList.push($scope.MasterGoalList[i]);
            }
        }
    }
