...
//                    , 'region_id' => $region_id
    ...
    private function getMonthDetailList($condition, $regions) {
        ...
        $MasterList = MasterGoalService::getList('Y', 'จำหน่ายน้ำเชื้อแช่แข็ง');
        ...
        foreach ($MasterList as $key => $value) {
            ...
            // get unit list by id
            // $UnitList = SpermSaleService::getUnitList($years, $months, $region_id, $value['id']);
            // print_r($unit_value);
            $data = [];
            ...
            foreach ($CooperativeList as $co_key => $co_value) {
                ...
                $detail_data = SpermSaleService::getDetailData($years, $months, $region_id, $cooperative_id, $value['id']);
