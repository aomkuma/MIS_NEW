    ...
    public function getMonthDataList($condition) {
        ...
        for ($i = 0; $i < $diffMonth; $i++) {
            ...
            foreach ($LossType as $key => $value) {
                ...
                $data = [];
                $data['LostWaitSaleName'] = $value['name'];
                $data['bg_color'] = '#ccc';
                ...
                foreach ($MilkType as $_key => $_value) {
                    ...
                    $MasterGoalList = MasterGoalService::getGoalIDByKeyword($goal_name, 'Y', 'การสูญเสียรอจำหน่าย', $factory_id);

                    if(!empty($MasterGoalList)){
                        // print_r($MasterGoalList);exit;
                        // $main_data = [];
                        $data = [];
                        $data['LostWaitSaleName'] = $_value['name'];
                        ...
                        foreach ($MasterGoalList as $k => $v) {
                            ...
                            // array_push($main_data['sub_data'], $data);
                            array_push($DataList, $data);
                        ...
                        $data = [];
                        $data['LostWaitSaleName'] = 'รวม '.$value['name'] . ' ' . $_value['name'];
                ...
                if(!empty($MasterGoalList)){
                    $data = [];
                    $data['LostWaitSaleName'] = 'รวม '.$value['name'] . 'ทั้งหมด';
        ...
        $data = [];
        $data['LostWaitSaleName'] = 'รวมสูญเสียระหว่างรอจำหน่าย ' . $MilkType[0]['name'] . ' ทั้งหมด';
        ...
        $data = [];
        $data['LostWaitSaleName'] = 'รวมสูญเสียระหว่างรอจำหน่าย ' . $MilkType[1]['name'] . ' ทั้งหมด';
        ...
        // $data['RegionName'] = $value['RegionName'];
        $data['bg_color'] = '#a9e8b3';
        $data['LostWaitSaleName'] = 'รวมสูญเสียระหว่างรอจำหน่ายทั้งสิ้น';
        ...
        $data = [];
        $data['LostWaitSaleName'] = 'ผลิตภัณฑ์ที่ผลิตได้ ';
        ...
        $data = [];
        $data['LostWaitSaleName'] = 'ร้อยละสูญเสียระหว่างรอจำหน่าย ';
    ...
    public function uploadData($request, $response, $args) {
        ...
        try {
            ...
            // exit;
                ...
                // exit;
                $data['result_package_amount'] = str_replace(',', '', $value['result_package_amount']);
                $data['result_thb'] = str_replace(',', '', $value['result_thb']);
