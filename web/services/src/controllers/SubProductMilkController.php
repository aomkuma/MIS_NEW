...
//                 die();
            ...
            if (empty($result)) {
                ...
                foreach ($ProductMilkDetail as $key1 => $value1) {
                    ...
                    $old_goal_name = $OldData['proname'] . ' - ' . $OldData['product_character'] . ' ' . $OldData['subname'] . ' - ' . $value1['name']  . ' ' . $value1['number_of_package'] . ' ' . $value1['unit'] . ' ' . (empty($value1['amount'] || $value1['amount'] == 0)?'0':$value1['amount']) . ' ' . $value1['amount_unit'] . ' ' . $value1['taste'];
                    $MasterGoal = MasterGoalService::getDataByName($old_goal_name);
                    // Add master goal
                    if(!empty($MasterGoal)){
                        $MasterGoal['goal_name'] = $OldData['proname'] . ' - ' . $_Data['product_character'] . ' ' . $_Data['name'] . ' - ' . $value1['name']  . ' ' . $value1['number_of_package'] . ' ' . $value1['unit'] . ' ' . (empty($value1['amount'] || $value1['amount'] == 0)?'0':$value1['amount']) . ' ' . $value1['amount_unit'] . ' ' . $value1['taste'];
